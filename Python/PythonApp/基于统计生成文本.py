...
#统计分析文本内容。首先需要使用ngram模型来把文章进行分词并统计频率。
#因为文章生成主要依据马尔可夫模型，所以使用了2-gram，这样可以统计出一个单词出现在另一个单词后的概率。
#生成新文章是基于分析大量随机事件的马尔可夫模型。随机事件的特点是在一个离散事件发生之后，
#另一个离散事件将在前一个事件的条件下以一定的概率发生。
#————————————————
